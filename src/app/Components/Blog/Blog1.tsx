...
const Blog1 = () => {
        ...
        const fetchBlogs = async () => {
            try {
                ...
                if (data.blogs && data.blogs.length > 0) {
                    ...
                    setBlogs([
                        {id: 1, title: 'Discuss The Three Main Types of Flooring', content: 'There are many z variationst anything but the lore majority a of passages available.', excerpt: 'There are many z variationst anything but the lore majority a of passages available.', image: '/assets/img/post_1.jpg', slug: 'blog-details', category: 'Business', author: 'Admin', published_date: new Date().toISOString(), created_at: new Date().toISOString()},
                        {id: 2, title: 'How to Keep Your Floors Safe During a Transfer', content: 'There are many z variationst anything but the lore majority a of passages available.', excerpt: 'There are many z variationst anything but the lore majority a of passages available.', image: '/assets/img/post_2.jpg', slug: 'blog-details', category: 'Business', author: 'Admin', published_date: new Date().toISOString(), created_at: new Date().toISOString()},
                    ]);
                }
            } catch (error) {
    ...
    const formatDate = (dateString: string) => {
        if (!dateString) return { day: '19', month: 'Dec' };
        ...
        {loading ? (
          ...
          <div className="cs_slider_container" data-autoplay="0" data-loop="1" data-speed="600" data-center="0" data-variable-width="0" data-xs-slides="1" data-sm-slides="1" data-md-slides="1" data-lg-slides="2" data-add-slides="2" data-slides-per-view="responsive">
            ...
            {blogs.map((blog) => {
              ...
              return (
              <div key={blog.id} className="cs_slide">
                <article className="cs_post cs_style_1 position-relative">
                  <Link href={blogLink} aria-label="Click to read post" className="cs_post_thumbnail cs_radius_15">
                   <Image src={blog.image || '/assets/img/post_1.jpg'} alt={blog.title} width={229} height={341}   />
                  <span className="cs_posted_by cs_radius_4 text-center position-absolute">
                  <span className="cs_date cs_orange_bg cs_white_color">{dateInfo.day}</span>
                  <span className="cs_month cs_white_bg cs_heading_color text-uppercase">{dateInfo.month}</span>
                  </span>
                  </Link>
                  <div className="cs_post_content cs_white_bg cs_radius_15">
                    <div className="cs_post_meta_wrapper cs_mb_8">
                      ...
                      <div className="cs_post_meta">
                        ...
                        <span>{blog.category || 'Business'}</span>
                      </div>
                    </div>
                    <h3 className="cs_post_title cs_fs_20 cs_semibold cs_mb_7"><Link href={blogLink} aria-label="Click to read post">{blog.title}</Link></h3>
                    <p className="cs_post_subtitle">{blog.excerpt || blog.content || 'There are many z variationst anything but the lore majority a of passages available.'}</p>
                    <Link href={blogLink} aria-label="Click to read post" className="cs_post_btn">
                      <span>Read More</span>
                      <span className="cs_accent_color">
